...
export class CookieService {
	...
	constructor(private readonly config: ConfigService<AllConfigs>) {
		this.isDevelopment = this.config.get('app.node_env', {infer: true}) === Environment.Development;	
		this.cookieDomain = this.config.get('app.cookie_domain', {infer: true}) as string;

		const cookie_expire_ttl = this.config.get('app.cookie_expire_ttl', {infer: true}) as number;
		this.expireDate = new Date(Date.now() + Number(cookie_expire_ttl))
	}

	public setCookie(response: Response, key: string, value: string, expires?: Date) {
